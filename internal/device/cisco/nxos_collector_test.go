...
// TestNewNXOSCollector tests NXOS collector creation
func TestNewNXOSCollector(t *testing.T) {
	target := "192.168.1.1"
	username := "admin"
	password := "password"
	timeout := 30 * time.Second

	collector := NewNXOSCollector(target, username, password, timeout)

	if collector == nil {
		t.Fatal("NewNXOSCollector() returned nil")
	}
...
// TestNXOSCollector_CommandSet tests command set configuration
func TestNXOSCollector_CommandSet(t *testing.T) {
	collector := NewNXOSCollector("192.168.1.1", "admin", "password", 30*time.Second)
...
// TestNXOSCollector_ValidateConnection tests connection validation
func TestNXOSCollector_ValidateConnection(t *testing.T) {
	collector := NewNXOSCollector("192.168.1.1", "admin", "password", 30*time.Second)
...
// TestNXOSCollector_BasicCommands tests basic command set
func TestNXOSCollector_BasicCommands(t *testing.T) {
	collector := NewNXOSCollector("192.168.1.1", "admin", "password", 30*time.Second)
