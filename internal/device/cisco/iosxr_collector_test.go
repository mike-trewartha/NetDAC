...
// TestNewIOSXRCollector tests IOS-XR collector creation
func TestNewIOSXRCollector(t *testing.T) {
	target := "192.168.1.1"
	username := "admin"
	password := "password"
	timeout := 30 * time.Second

	collector := NewIOSXRCollector(target, username, password, timeout)

	if collector == nil {
		t.Fatal("NewIOSXRCollector() returned nil")
	}
...
// TestIOSXRCollector_CommandSet tests command set configuration
func TestIOSXRCollector_CommandSet(t *testing.T) {
	collector := NewIOSXRCollector("192.168.1.1", "admin", "password", 30*time.Second)
...
// TestIOSXRCollector_ValidateConnection tests connection validation
func TestIOSXRCollector_ValidateConnection(t *testing.T) {
	collector := NewIOSXRCollector("192.168.1.1", "admin", "password", 30*time.Second)
...
// TestIOSXRCollector_BasicCommands tests basic command set
func TestIOSXRCollector_BasicCommands(t *testing.T) {
	collector := NewIOSXRCollector("192.168.1.1", "admin", "password", 30*time.Second)
