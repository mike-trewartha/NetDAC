...
// TestNewFTDCollector tests FTD collector creation
func TestNewFTDCollector(t *testing.T) {
	target := "192.168.1.1"
	username := "admin"
	password := "password"
	timeout := 30 * time.Second

	collector := NewFTDCollector(target, username, password, timeout)

	if collector == nil {
		t.Fatal("NewFTDCollector() returned nil")
	}
...
// TestFTDCollector_CommandSet tests command set configuration
func TestFTDCollector_CommandSet(t *testing.T) {
	collector := NewFTDCollector("192.168.1.1", "admin", "password", 30*time.Second)
...
// TestFTDCollector_ValidateConnection tests connection validation
func TestFTDCollector_ValidateConnection(t *testing.T) {
	collector := NewFTDCollector("192.168.1.1", "admin", "password", 30*time.Second)
...
// TestFTDCollector_ForensicCommands tests forensic command categories
func TestFTDCollector_ForensicCommands(t *testing.T) {
	collector := NewFTDCollector("192.168.1.1", "admin", "password", 30*time.Second)
...
// TestFTDCollector_BasicCommands tests basic command set
func TestFTDCollector_BasicCommands(t *testing.T) {
	collector := NewFTDCollector("192.168.1.1", "admin", "password", 30*time.Second)
